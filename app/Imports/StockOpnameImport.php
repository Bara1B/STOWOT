...
class StockOpnameImport implements ToModel, WithHeadingRow, WithBatchInserts, WithChunkReading
{
    ...
    public function model(array $row)
    {
        ...
        $lotSerial  = $row['lotserial'] ?? $row['lot_serial'] ?? $row['serial'] ?? '';
        ...
        // Skip empty rows
        if (empty($itemNumber) && empty($row['description'])) {
            return null;
        }

        return new StockOpname([
            'file_id' => $this->fileId,
            'location_system' => $row['location_system'] ?? '',
            'location_actual' => $row['location_actual'] ?? '',
