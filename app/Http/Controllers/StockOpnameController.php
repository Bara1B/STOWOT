...
class StockOpnameController extends Controller
{
    ...
    public function showData($fileId)
    {
        ...
        $stockOpnames = DB::table('stock_opnames')
            ->leftJoin('overmate', function($join) {
                $join->on('stock_opnames.item_number', '=', 'overmate.item_number');
            })
            ->select(
                'stock_opnames.*',
                'overmate.overmate_qty',
                'overmate.nama_bahan as overmate_nama_bahan',
                'overmate.manufactur as overmate_manufactur',
                DB::raw('CAST(COALESCE(stock_opnames.stok_fisik, 0) - COALESCE(stock_opnames.quantity_on_hand, 0) AS DECIMAL(15,5)) as selisih'),
