...
class StockOpnameFilledExport implements FromQuery, WithMapping, WithHeadings, WithColumnWidths, WithStyles
{
    ...
    public function query(): Builder
    {
        return DB::table('stock_opnames as so')
            ...
            ->select([
                'so.location_system',
                'so.location_actual_status',
                'so.location_actual_keterangan',
                'so.item_number',
                'so.description',
                'so.unit_of_measure',
                'so.lot_serial',
    ...
    public function map($r): array
    {
        ...
        if (!is_null($r->stok_fisik) && !is_null($r->quantity_on_hand)) {
            ...
            $masuk = ($selisih <= $overmate) ? 'Iya' : 'Tidak';
        }
        ...
        // Location Actual logic
        $locationActual = '';
        if ($r->location_actual_status === 'centang') {
            $locationActual = '✓';
        } elseif ($r->location_actual_status === 'x') {
            $locationActual = $r->location_actual_keterangan ?: 'X';
        }

        return [
            '', // No (left blank for performance without state)
            $r->location_system,
            $locationActual, // Location Actual: shows ✓, keterangan, or blank
            $r->item_number,
            $r->description,
            $r->unit_of_measure,
            $r->lot_serial,
            ...
            $masuk,
            $r->keterangan ?: '', // Keterangan from database
        ];
    }
